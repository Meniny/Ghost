...
extension URLSessionConfiguration {
    ...
    public class Builder {
        ...
        public enum GhostMultipathServiceType: Int {
        ...
        public private(set) var multipathServiceType: GhostMultipathServiceType?

        public init(_ configuration: URLSessionConfiguration? = nil) {

            cache = GhostRequest.GhostCachePolicy(rawValue: configuration?.requestCachePolicy.rawValue ?? 0)

            requestTimeout = configuration?.timeoutIntervalForRequest

            resourceTimeout = configuration?.timeoutIntervalForResource

            serviceType = GhostRequest.GhostServiceType(rawValue: configuration?.networkServiceType.rawValue ?? 0)
            ...
            protocolClasses = configuration?.protocolClasses
                ...
                if #available(iOS 11.0, *) {
                    self.multipathServiceType = GhostMultipathServiceType(rawValue: configuration?.multipathServiceType.rawValue ?? 0)
                }
            #endif
        }

        @discardableResult open func setCache(_ cachePolicy: GhostRequest.GhostCachePolicy?) -> Self {
            self.cache = cache
            return self
        }
        ...
        @discardableResult open func setMultipathServiceType(_ multipathServiceType: GhostMultipathServiceType?) -> Self {
